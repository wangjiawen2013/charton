...
- **Space Reservation**: The layout engine asks the guides how much room they need and turns that
  into bands around the plot panel. A legend may never be longer than the panel it sits next to;
  that is the rule which keeps a long legend from covering the axis labels. The bands and the
  reservation loop are described in [Space Manager](space.md).
- **Anchoring**: A guide is drawn inside the exact band that was reserved for it, so the reserved
  space and the drawn position always agree.
