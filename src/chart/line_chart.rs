...
impl Chart<MarkLine> {
    ...
    fn render_lines(&self, svg: &mut String, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        ...
        let mark = self.mark.as_ref()
            .ok_or_else(|| ChartonError::Internal("Mark should exist when rendering lines".to_string()))?;
        
        // Create or get color column data
        let color_series = if let Some(color_enc) = &self.encoding.color {
            // Use existing color column
            self.data.column(&color_enc.field)?.with_name("color".into())
        } else {
        ...
        let global_x_min = processed_data.x_transformed_vals.iter().copied().fold(f64::INFINITY, f64::min);
        let global_x_max = processed_data.x_transformed_vals.iter().copied().fold(f64::NEG_INFINITY, f64::max);
        ...
        for (group_index, (_group_name, indices)) in group_data.iter().enumerate() {
            ...
            let group_x_vals: Vec<f64> = indices.iter()
                .map(|&i| processed_data.x_transformed_vals[i])
                .collect();
            let group_y_vals: Vec<f64> = indices.iter()
            ...
            let (mut final_x_vals, mut final_y_vals) = if mark.use_loess {
                crate::stats::stat_loess::loess(
                    &group_x_vals,
                    &group_y_vals,
                    mark.loess_bandwidth
                )
            } else {
            ...
            if matches!(mark.interpolation, PathInterpolation::StepAfter) {
                ...
                let last_x = *final_x_vals.last()
                    .ok_or_else(|| ChartonError::Internal("Empty x values when rendering ECDF line".to_string()))?;
                if last_x != global_x_max {
                    ...
                    let last_y = *final_y_vals.last()
                        .ok_or_else(|| ChartonError::Internal("Empty y values when rendering ECDF line".to_string()))?;
                    final_y_vals.push(last_y);
                }
            }

            // Create points for this group, accounting for swapped axes
            let points: Vec<(f64, f64)> = final_x_vals.iter()
