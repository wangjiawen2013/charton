...
impl Chart<MarkPoint> {
    ...
    fn render_points(&self, svg: &mut String, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        // Process chart data
        ...
        for i in 0..processed_data.x_vals.len() {
            ...
            let shape = if let Some(ref shape_vals) = processed_data.shape_vals {
                ...
                let unique_shapes_series = Series::new("shape_vals".into(), shape_vals.clone())
                    .unique_stable()?;
                ...
                let shape_index = unique_shapes.iter()
                    .position(|x| x == shape_str)
                    .ok_or_else(|| ChartonError::Internal("shape not found in unique values".to_string()))?;
            ...
            let size = if let Some(ref normalized_sizes) = processed_data.normalized_sizes {
                *normalized_sizes.get(i)
                    .ok_or_else(|| ChartonError::Internal("Index out of bounds in normalized_sizes".to_string()))?
            } else {
                mark.size
            };
...
impl crate::chart::common::LegendRenderer for Chart<MarkPoint> {
    fn render_legends(&self, svg: &mut String, theme: &Theme, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        ...
        size_legend_renderer::render_size_legend(
            svg,
            self,
            theme,
            context
        ...
        shape_legend_renderer::render_shape_legend(
            svg,
            self,
            theme,
            context
