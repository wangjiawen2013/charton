...
impl Chart<MarkArea> {
    ...
    fn render_areas(&self, svg: &mut String, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        ...
        for (group_index, (_group_name, indices)) in group_data.iter().enumerate() {
            ...
            let mut points: Vec<(f64, f64)> = group_x_vals.iter()
        ...
        let should_draw_zero_line = self.encoding.y.as_ref()
