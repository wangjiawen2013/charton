...
impl Chart<MarkArc> {
    ...
    fn render_arcs(&self, svg: &mut String, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        ...
        let working_df = {
            ...
            let grouped_colors: Vec<&str> = grouped_color_series.str()?.into_no_null_iter().collect();
        ...
        let theta_series = working_df_source
            .column(self.encoding.theta.as_ref().unwrap().field.as_str())?;
        ...
        for (i, &value) in theta_vals.iter().enumerate() {
            ...
            let mark = self.mark.as_ref()
                .ok_or_else(|| ChartonError::Internal("Mark should exist when rendering arcs".to_string()))?;
