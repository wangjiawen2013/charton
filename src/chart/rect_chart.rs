...
impl Chart<MarkRect> {
    ...
    fn render_rects(&self, svg: &mut String, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        ...
        let mark = self.mark.as_ref()
            .ok_or_else(|| ChartonError::Internal("Mark should exist when rendering rectangles".to_string()))?;
        ...
        let x_min = x_transformed_vals.iter().copied().fold(f64::INFINITY, f64::min);
        let x_max = x_transformed_vals.iter().copied().fold(f64::NEG_INFINITY, f64::max);
        let x_unique_count: HashSet<OrderedFloat<f64>> = x_transformed_vals.clone().into_iter().map(OrderedFloat).collect();

        // Calculate min, max, and unique count for y values using transformed data
        let y_min = y_transformed_vals.iter().copied().fold(f64::INFINITY, f64::min);
        let y_max = y_transformed_vals.iter().copied().fold(f64::NEG_INFINITY, f64::max);
        let y_unique_count: HashSet<OrderedFloat<f64>> = y_transformed_vals.clone().into_iter().map(OrderedFloat).collect();

        // Calculate rectangle width and height
        let (rect_h_pixels, rect_v_pixels) = {
            // When axes are swapped, we need to adjust how we calculate dimensions
            let x_pixels = ((context.x_mapper)(x_max) - (context.x_mapper)(x_min)).abs() / (x_unique_count.len() as f64 - 1.0);
            let y_pixels = ((context.y_mapper)(y_max) - (context.y_mapper)(y_min)).abs() / (y_unique_count.len() as f64 - 1.0);
            if context.swapped_axes  { (y_pixels, x_pixels) } else { (x_pixels, y_pixels) }
        };

        // Render rectangles
        for (i, (&x_val, &y_val)) in x_transformed_vals.iter().zip(&y_transformed_vals).enumerate() {
            ...
            let (adjusted_h_pixel, adjusted_v_pixel) = (h_pixel - rect_h_pixels / 2.0, v_pixel - rect_v_pixels / 2.0);
            ...
            let stroke_color_str = mark.stroke
                .as_ref()
                .map(|c| c.get_color())
