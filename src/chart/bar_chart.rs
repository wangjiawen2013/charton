...
impl Chart<MarkBar> {
    ...
    fn render_bars(&self, svg: &mut String, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        ...
        let mark = self.mark.as_ref()
            .ok_or_else(|| ChartonError::Internal("Mark should exist when rendering bars".to_string()))?;
        ...
        let should_stack = self.encoding.y.as_ref().map(|y| y.stack)
            .ok_or_else(|| ChartonError::Internal("Y encoding should exist when rendering bars".to_string()))?;
        ...
        let (group_vals, value_vals) = (&processed_data.x_transformed_vals, &processed_data.y_transformed_vals);
        ...
        for (_group_idx, group_df) in groups.iter().enumerate() {
            ...
            let colors: Vec<String> = color_col.str()?.into_no_null_iter().map(|s| s.to_string()).collect();
            ...
            if should_stack {
                ...
                for (idx, (color_label, _value)) in colors.iter().zip(values.iter()).enumerate() {
                    ...
                    let fill_color = if self.encoding.color.is_some() {
                        ...
                        let color_index = unique_colors.iter().position(|c| c == color_label)
                ...
                let bar_width_data = if matches!(group_scale, crate::coord::Scale::Discrete) {
                    ...
                    if group_values.len() > 1 {
                        ...
                        let max_group = group_values.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
                ...
                for (sub_idx, (color_label, value)) in colors.iter().zip(values.iter()).enumerate() {
                    ...
                    let offset = (sub_idx as f64 - (groups_count - 1.0) / 2.0) * (bar_width_pixels + spacing_pixels);
        ...
        if should_draw_zero_line {
            if !self.swapped_axes {
                ...
                writeln!(
                    ...
                    context.draw_x0, y_zero, context.draw_x0 + context.plot_width, y_zero
                )?;
            } else {
                ...
                writeln!(
                    svg,
                    r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="black" stroke-dasharray="5,5" />"#,
                    x_zero, context.draw_y0, x_zero, context.draw_y0 + context.plot_height
                )?;
            }
        }
