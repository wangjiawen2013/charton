...
/// This struct provides a shared rendering context for chart layers.
/// 
/// It contains essential information needed during the rendering process,
...
pub struct SharedRenderingContext<'a> {
    ...
    pub(crate) y_mapper: &'a dyn Fn(f64) -> f64,
    pub(crate) coord_system: &'a Cartesian,
    pub(crate) draw_x0: f64,
    pub(crate) draw_y0: f64,
...
impl<T: Mark> Chart<T> {
    ...
    where S: TryInto<DataFrameSource, Error = ChartonError>,
    {
        ...
        chart.data = Chart::<T>::convert_numeric_types(chart.data.clone())?;
        ...
        U: IntoEncoding,
    {
        ...
        for (col_name, dtype) in schema.iter() {
            ...
            match dtype {
                ...
                UInt8 | UInt16 | UInt32 | UInt64 | Int8 | Int16 | Int32 | Int64 | Int128 | Float32 | Float64 | String => {
                    // These types are supported, continue
                },
                // Unsupported types
                _ => {
                    return Err(ChartonError::Data(
                        format!("Column '{}' has unsupported data type {:?}. Only numeric types and String are supported.", 
                            col_name, dtype)
                    ));
                }
            }
        ...
        let mark = self.mark.as_ref().ok_or_else(|| {
            ChartonError::Mark("A mark is required to create a chart".into())
        })?;
        ...
        match mark.mark_type() {
            ...
            "errorbar" | "bar" | "hist" | "line" | "point" | "area" | "boxplot" | "text" | "rule" => {
                if self.encoding.x.is_none() || self.encoding.y.is_none() {
                    return Err(ChartonError::Encoding(
                        format!("{} chart requires both x and y encodings", mark.mark_type())
                    ));
                }
            }
            // Rect charts require x, y, and color encodings
            ...
            _ => {
                return Err(ChartonError::Mark(
                    format!("Unknown mark type: {}. This is a programming error - all mark types should be handled explicitly.", mark.mark_type())
                ));
            }
        }
        ...
        if mark.mark_type() == "errorbar" {
            // we've already converted all numeric types to f64 when building the chart
            expected_types.insert(self.encoding.y.as_ref().unwrap().field.as_str(), vec![DataType::Float64]);
        ...
        if mark.mark_type() == "hist" {
            active_fields.retain(|&field| field != self.encoding.y.as_ref().unwrap().field.as_str());
            // we've already converted all numeric types to f64 when building the chart
            expected_types.insert(self.encoding.x.as_ref().unwrap().field.as_str(), vec![DataType::Float64]);
        }

        // Add type checking for rect charts - color encoding must be f64 (continuous) for proper color mapping
        if mark.mark_type() == "rect" {
            // we've already converted all numeric types to f64 when building the chart
            expected_types.insert(self.encoding.color.as_ref().unwrap().field.as_str(), vec![DataType::Float64]);
        }
        ...
        if mark.mark_type() == "boxplot" {
            ...
            expected_types.insert(self.encoding.x.as_ref().unwrap().field.as_str(), vec![DataType::String]);
            expected_types.insert(self.encoding.y.as_ref().unwrap().field.as_str(), vec![DataType::Float64]);
        }
        ...
        check_schema(&mut self.data.df, &active_fields, &expected_types)
            .map_err(|e| {
        ...
        let filtered_df = self.data.df.drop_nulls(Some(
            ...
            eprintln!("Error filtering null values from columns {:?}: {}", active_fields, e);
            e
        })?;

        // Check if the filtered DataFrame is empty
        if filtered_df.height() == 0 {
            eprintln!("Warning: No valid data remaining after filtering null values from columns: {:?}", active_fields);
...
/// Trait for layers in a layered chart.
/// 
...
pub trait Layer: MarkRenderer + LegendRenderer {
    ...
    fn get_x_scale(&self) -> Result<Option<Scale>, ChartonError>;
    fn get_y_scale(&self) -> Result<Option<Scale>, ChartonError>;

    // Method to calculate legend width for this layer
    fn calculate_legend_width(&self, theme: &Theme, chart_height: f64, top_margin: f64, bottom_margin: f64) -> f64;
...
/// 
/// The `MarkRenderer` trait defines the interface for rendering the visual
/// elements (marks) of a chart, such as points, bars, lines, etc. Implementors
/// of this trait can render their specific mark types to an SVG string using
/// the provided rendering context.
/// 
/// This trait is typically implemented by chart types to define how their
/// data should be visually represented in the chart area.
...
/// 
/// The `LegendRenderer` trait defines the interface for rendering the legend
/// elements of a chart. Implementors of this trait can render their specific
/// legend types to an SVG string using the provided rendering context and theme.
/// 
/// This trait is typically implemented by chart types to define how their
/// legend should be visually represented, including color legends, size legends,
/// and shape legends.
    ...
    Chart<T>: MarkRenderer + LegendRenderer,
{
    ...
    fn get_y_continuous_bounds(&self) -> Result<(f64, f64), ChartonError> {
        ...
        let (y_min_val, y_max_val) = match self.mark.as_ref().map(|m| m.mark_type()).as_deref() {
            ...
            Some("errorbar") => {
                ...
                let (y_min_field, y_max_field) = if let Some(y2_encoding) = &self.encoding.y2 {
                    ...
                    (
                        self.encoding.y.as_ref().map(|y| format!("__charton_temp_{}_min", y.field)).expect("Y encoding and column ymin required"),
                        self.encoding.y.as_ref().map(|y| format!("__charton_temp_{}_max", y.field)).expect("Y encoding and column ymax required")
                    )
                };
            ...
            Some("bar") if y_encoding.stack && self.encoding.color.is_some() => {
                ...
                let grouped_sums = df.lazy()
                    .group_by_stable([col(group_col)])
                    .agg([
                        col(&y_encoding.field).sum().alias("sum_values")
                    ])
                    .collect()?;
        ...
        let (final_min, final_max) = match y_encoding.zero {
            ...
            None => {
                let is_supported_mark = self.mark.as_ref()
                    .map(|m| {
                        m.mark_type() == "bar" ||
                        m.mark_type() == "hist" ||
                        m.mark_type() == "area"
    ...
    fn get_x_discrete_tick_labels(&self) -> Result<Option<Vec<String>>, ChartonError> {
        ...
        let x_encoding = self.encoding.x.as_ref().expect("X encoding should exist");
        let unique_labels = self.data.column(&x_encoding.field)?
            .unique_stable()?
            .str()?
            .into_no_null_iter()
            .map(|s| s.to_string())
    ...
    fn get_y_discrete_tick_labels(&self) -> Result<Option<Vec<String>>, ChartonError> {
        ...
        let y_encoding = self.encoding.y.as_ref().expect("Y encoding should exist");
        let unique_labels = self.data.column(&y_encoding.field)?
            .unique_stable()?
            .str()?
            .into_no_null_iter()
            .map(|s| s.to_string())
    ...
    fn get_x_scale(&self) -> Result<Option<Scale>, ChartonError> {
        ...
        let x_encoding = self.encoding.x.as_ref().unwrap();
        let x_series = self.data.column(x_encoding.field.as_str())?;
        let scale = x_encoding.scale.as_ref().map_or_else(
    ...
    fn get_y_scale(&self) -> Result<Option<Scale>, ChartonError> {
        ...
        let y_encoding = self.encoding.y.as_ref().unwrap();
        let y_series = self.data.column(y_encoding.field.as_str())?;
        let scale = y_encoding.scale.as_ref().map_or_else(
            || crate::data::determine_scale_for_dtype(y_series.dtype()),
            |s| s.clone()
        );
    ...
    fn calculate_legend_width(&self, theme: &Theme, chart_height: f64, top_margin: f64, bottom_margin: f64) -> f64 {
        ...
        if let Some(color_enc) = &self.encoding.color {
            let color_series = self.data.column(&color_enc.field).expect("Color column should exist");
            ...
            if matches!(scale_type, Scale::Discrete) {
                ...
                let unique = color_series
                    .unique_stable().expect("Failed to calculate legend width")
                    .str().expect("Failed to calculate legend width")
                    .into_no_null_iter()
                    .map(|s| s.to_string())
                    .collect::<Vec<String>>();
                ...
                let columns_needed = ((total_items as f64) / (max_items_per_column as f64)).ceil() as usize;
        ...
        if let Some(shape_enc) = &self.encoding.shape {
            let shape_series = self.data.column(&shape_enc.field).expect("Shape column should exist");
            ...
            let max_label_width = unique_shapes.iter()
                .map(|label| estimate_text_width(label, theme.tick_label_font_size as f64))
                .fold(0.0, f64::max);
...
impl LayeredChart {
    ...
    fn effective_right_margin(&self) -> f64 {
        ...
        if initial_plot_w < min_plot_width {
            ...
            if new_plot_w >= min_plot_width {
                ...
                let final_right_margin = base_right_margin_width + (additional_width_needed - actual_compression);
    ...
    fn create_shared_coord_system(&self) -> Result<Cartesian, ChartonError> {
            ...
            if let Some(ref explicit_labels) = self.x_tick_labels {
                ...
                let explicit_ticks: Vec<(f64, String)> = explicit_labels
                    ...
                    .filter_map(|label| {
                        ...
                        x_axis.automatic_ticks.ticks.iter().position(|tick| tick.label == *label)
        ...
        if matches!(y_scale, Scale::Discrete) {
            ...
            if let Some(ref explicit_labels) = self.y_tick_labels {
                ...
                let explicit_ticks: Vec<(f64, String)> = explicit_labels
                    ...
                    .filter_map(|label| {
                        // Find the position of this label in the automatic ticks
                        y_axis.automatic_ticks.ticks.iter().position(|tick| tick.label == *label)
                            .map(|index| (index as f64, label.clone()))
                    })
                    .collect();
        ...
        let coord_system = Cartesian::new(x_axis, y_axis, x_axis_padding_min, x_axis_padding_max, y_axis_padding_min, y_axis_padding_max);
    ...
    fn render(&self, svg: &mut String) -> Result<(), ChartonError> {
        ...
        let (x_mapper, y_mapper): (Box<dyn Fn(f64) -> f64>, Box<dyn Fn(f64) -> f64>) = if swapped_axes {
            // When axes are swapped, x maps to vertical pixels and y maps to horizontal pixels
            let x_mapper: Box<dyn Fn(f64) -> f64> = Box::new(coord_system.x_data_to_vertical_pixels(draw_y0, plot_h));
            let y_mapper: Box<dyn Fn(f64) -> f64> = Box::new(coord_system.y_data_to_horizontal_pixels(draw_x0, plot_w));
            (x_mapper, y_mapper)
        } else {
            // Normal orientation: x maps to horizontal pixels and y maps to vertical pixels
            let x_mapper: Box<dyn Fn(f64) -> f64> = Box::new(coord_system.x_data_to_horizontal_pixels(draw_x0, plot_w));
            let y_mapper: Box<dyn Fn(f64) -> f64> = Box::new(coord_system.y_data_to_vertical_pixels(draw_y0, plot_h));
            (x_mapper, y_mapper)
        };
