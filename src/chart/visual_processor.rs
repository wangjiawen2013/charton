...
use crate::coordinate::cartesian::Cartesian2D; // Match your struct name
...
impl VisualProcessedData {
    ...
    pub(crate) fn new<T: crate::mark::Mark>(
        ...
        coord_system: &Cartesian2D, // Use Cartesian2D to match your code
        ...
        let x_field = chart.encoding.x.as_ref()
            .ok_or_else(|| ChartonError::Encoding("X encoding is required".into()))?.field.as_str();
        let y_field = chart.encoding.y.as_ref()
            .ok_or_else(|| ChartonError::Encoding("Y encoding is required".into()))?.field.as_str();

        let x_series = chart.data.column(x_field)?;
        let y_series = chart.data.column(y_field)?;

        // Access the boxed scales directly using .x_scale and .y_scale
        ...
        // --- 4. Size Aesthetic ---
        let sizes = if let Some(size_enc) = &chart.encoding.size {
