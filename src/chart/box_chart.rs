...
impl Chart<MarkBoxplot> {
    ...
    fn calculate_grouped_box_stats(&self) -> Result<Vec<(String, String, Option<BoxStats>)>, ChartonError> {
        ...
        let color_field = self.encoding.color
            .as_ref()
        ...
        let working_df = if self.encoding.color.is_none() {
            let temp_color_series = Series::new(
                (&color_field).into(), 
        ...
        let group_values: Vec<String> = unique_groups.str()?.into_no_null_iter()
            .map(|s| {
        ...
        let color_values = {
            ...
            let values: Vec<String> = unique.str()?.into_no_null_iter()
                .map(|s| {
                    let s_str = s.to_string();
                    // Remove quotes if they exist
                    if s_str.starts_with('"') && s_str.ends_with('"') && s_str.len() >= 2 {
        ...
        let expected_combinations: Vec<(String, String)> = itertools::iproduct!(
            group_values.iter().cloned(),
            color_values.iter().cloned()
        ).collect();

        // Group by both group and color columns (since we always have a color column now)
        let grouped_stats = working_df.clone()
            .lazy()
            .group_by_stable([col(group_col), col(&color_field)])
            .agg([
                col(value_col).min().alias("min"),
                col(value_col).quantile(lit(0.25), QuantileMethod::Linear).alias("q1"),
                col(value_col).median().alias("median"),
                col(value_col).quantile(lit(0.75), QuantileMethod::Linear).alias("q3"),
        ...
        let stats_with_bounds = grouped_stats.lazy()
            .with_columns([
        ...
        for i in 0..group_series.len() {
            ...
            let clean_group_value = if group_value.starts_with('"') && group_value.ends_with('"') && group_value.len() >= 2 {
            ...
            let clean_color_value = if color_value.starts_with('"') && color_value.ends_with('"') && color_value.len() >= 2 {
    ...
    fn render_box_whiskers(&self, svg: &mut String, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        ...
        let unique_groups: Vec<String> = box_stats.iter()
        ...
        for (group_idx, group_label) in sorted_groups.iter().enumerate() {
            ...
            let group_boxes: Vec<_> = box_stats.iter()
            ...
            let box_width_data = user_width.min(span / (groups_count + (groups_count - 1.0) * spacing));
            ...
            for (sub_idx, (_, color_label, stats_opt)) in group_boxes.iter().enumerate() {
                ...
                let offset_pixel = (sub_idx as f64 - (groups_count - 1.0) / 2.0) * (box_width_pixels + spacing_pixels);
                ...
                let fill_color = if self.encoding.color.is_some() {
                    ...
                    let color_index = box_stats.iter()
                        .position(|(_, c, _)| c == color_label)
                        .ok_or_else(|| ChartonError::Internal("color not found in unique values".to_string()))?;
                ...
                let outlier_y_coords: Vec<f64> = stats.outliers.iter()
