...
impl Chart<MarkErrorBar> {
    ...
    fn render_errorbars(&self, svg: &mut String, context: &SharedRenderingContext) -> Result<(), ChartonError> {
        // Process chart data using shared processor
        let processed_data = super::data_processor::ProcessedChartData::new(self, context.coord_system)?;
        ...
        let (y_min_field, y_max_field) = if self.encoding.y2.is_some() {
            ...
            (
                self.encoding.y.as_ref().map(|y| y.field.clone()).unwrap_or("y".to_string()),
                self.encoding.y2.as_ref().map(|y2| y2.field.clone()).unwrap_or("y".to_string())
            )
        } else {
            // When y2 encoding doesn't exist, use the auto-generated field names from transform_errorbar_data
            let y_field = self.encoding.y.as_ref().map(|y| y.field.clone()).unwrap_or("y".to_string());
            (
        ...
        let (y_mean_vals, y_min_vals, y_max_vals) = {
            ...
            let y_min_transformed: Vec<f64> = match context.coord_system.y_axis.scale {
                Scale::Log => {
                    y_min_series.f64()?.into_no_null_iter().map(|y| y.log10()).collect()
                },
                Scale::Linear | Scale::Discrete => y_min_series.f64()?.into_no_null_iter().collect()
            };

            let y_max_transformed: Vec<f64> = match context.coord_system.y_axis.scale {
                Scale::Log => {
                    y_max_series.f64()?.into_no_null_iter().map(|y| y.log10()).collect()
                },
                Scale::Linear | Scale::Discrete => y_max_series.f64()?.into_no_null_iter().collect()
            };
        ...
        let mark = self.mark.as_ref()
            .ok_or_else(|| ChartonError::Internal("Mark should exist when rendering error bars".to_string()))?;

        let stroke_color = mark.color.as_ref()
            .map(|c| c.get_color())
        ...
        for i in 0..x_vals.len() {
            ...
            if !context.swapped_axes {
                ...
                writeln!(
                    ...
                    x_center_pixel, y_min_pixel, x_center_pixel, y_max_pixel, stroke_color, mark.stroke_width, mark.opacity
                )?;
                ...
                writeln!(
                    svg,
                    r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" opacity="{}"/>"#,
                    x_center_pixel - mark.cap_length, y_max_pixel, x_center_pixel + mark.cap_length, y_max_pixel, stroke_color, mark.stroke_width, mark.opacity
                )?;
                ...
                writeln!(
                    svg,
                    r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" opacity="{}"/>"#,
                    x_center_pixel - mark.cap_length, y_min_pixel, x_center_pixel + mark.cap_length, y_min_pixel, stroke_color, mark.stroke_width, mark.opacity
                )?;
            } else {
                ...
                writeln!(
                    svg,
                    r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" opacity="{}"/>"#,
                    y_min_pixel, x_center_pixel, y_max_pixel, x_center_pixel, stroke_color, mark.stroke_width, mark.opacity
                )?;
                ...
                if mark.show_center {
                    writeln!(
                        svg,
                        r#"<circle cx="{}" cy="{}" r="3" fill="{}" opacity="{}"/>"#,
                        y_center_pixel, x_center_pixel, stroke_color, mark.opacity  // Swapped x and y coordinates
                    )?;
                }
                ...
                writeln!(
                    svg,
                    r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" opacity="{}"/>"#,
                    y_max_pixel, x_center_pixel - mark.cap_length, y_max_pixel, x_center_pixel + mark.cap_length, stroke_color, mark.stroke_width, mark.opacity
                )?;
                ...
                writeln!(
                    svg,
                    r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" opacity="{}"/>"#,
                    y_min_pixel, x_center_pixel - mark.cap_length, y_min_pixel, x_center_pixel + mark.cap_length, stroke_color, mark.stroke_width, mark.opacity
                )?;
            }
        }
...
impl LegendRenderer for Chart<MarkErrorBar> {
    fn render_legends(&self, _svg: &mut String, _theme: &crate::theme::Theme, _context: &SharedRenderingContext) -> Result<(), ChartonError> {
