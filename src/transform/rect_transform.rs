    ...
    pub(crate) fn transform_rect_data(mut self) -> Result<Self, ChartonError> {
        ...
        let processed_df = {
            ...
            if !x_is_discrete {
                ...
                let min_val = current_x_series.f64()?.min()
                    .expect("Internal error: Failed to calculate minimum value for rect chart data");
                let max_val = current_x_series.f64()?.max()
                    .expect("Internal error: Failed to calculate maximum value for rect chart data");
            ...
            if !y_is_discrete {
                ...
                let n_bins = if unique_count == 1 {
                    ...
                    y_encoding.bins.unwrap_or_else(|| {
                        ((unique_count as f64).sqrt() as usize).max(5).min(50)
                    })
                };

                // Get min and max values for binning using Polars' built-in methods
                let min_val = current_y_series.f64()?.min()
                    .expect("Internal error: Failed to calculate minimum value for rect chart data");
                let max_val = current_y_series.f64()?.max()
                    .expect("Internal error: Failed to calculate maximum value for rect chart data");
        ...
        let grouped_df = processed_df.lazy()
            .group_by_stable([col(&x_encoding.field), col(&y_encoding.field)])
            .agg([col(&color_encoding.field).sum().alias(&color_encoding.field)])
            .collect()?;
        ...
        let filled_df = if x_bin_labels.is_some() || y_bin_labels.is_some() {
            ...
            let all_x_bins = x_bin_labels.clone().unwrap_or_else(|| {
                ...
                grouped_df.column(&x_encoding.field)
            ...
            let all_y_bins = y_bin_labels.clone().unwrap_or_else(|| {
                // For discrete data, get unique values
                grouped_df.column(&y_encoding.field)
            ...
            let x_repeated: Vec<String> = all_x_bins.iter()
                .flat_map(|x| vec![x.clone(); all_y_bins.len()])
                .collect();
                
            let y_repeated: Vec<String> = all_y_bins.iter()
                .cycle()
        ...
        let final_df = {
            ...
            if let (Some(labels), Some(middles)) = (&x_bin_labels, &x_bin_middles) {
                ...
                let x_series = df.column(&x_encoding.field)
                    .unwrap_or_else(|e| panic!("Internal error: x column '{}' should exist: {:?}", x_encoding.field, e))
                    .str()
                    .unwrap_or_else(|e| panic!("Internal error: x column '{}' should be string type: {:?}", x_encoding.field, e));
            ...
            if let (Some(labels), Some(middles)) = (&y_bin_labels, &y_bin_middles) {
                ...
                let y_series = df.column(&y_encoding.field)
                    .unwrap_or_else(|e| panic!("Internal error: y column '{}' should exist: {:?}", y_encoding.field, e))
                    .str()
                    .unwrap_or_else(|e| panic!("Internal error: y column '{}' should be string type: {:?}", y_encoding.field, e));
