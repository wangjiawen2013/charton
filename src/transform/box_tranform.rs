...
use crate::mark::Mark;
use crate::core::data::{Dataset, ColumnVector, get_quantile};
    ...
    pub(crate) fn transform_boxplot_data(mut self) -> Result<Self, ChartonError> {
        ...
        for i in 0..row_count {
            ...
            let c_val = color_field_name.as_ref().map(|f| {
                self.data.column(f).unwrap().get_as_string(i).unwrap_or_else(|| "null".to_string())
            });
        ...
        if let Some(ref _f) = color_field_name {
            let mut unique_colors: Vec<String> = group_map.keys()
        ...
        let groups_count = if color_to_idx.is_empty() { 1.0 } else { color_to_idx.len() as f64 };
        ...
        let stats_results: Vec<_> = groups.into_par_iter().map(|((x_val, c_val), indices)| {
            ...
            for &v in &vals {
                if v < lower_fence || v > upper_fence {
                    ...
                    if v < whisker_min { whisker_min = v; }
                    if v > whisker_max { whisker_max = v; }
                }
            }

            let sub_idx = c_val.as_ref().and_then(|c| color_to_idx.get(c)).cloned().unwrap_or(0.0);

            (x_val, c_val, Some(BoxStats {
                q1, median, q3, whisker_min, whisker_max, outliers, sub_idx
            }))
        }).collect();
        ...
        new_ds.add_column(x_name, ColumnVector::String { data: final_x, validity: None })?;
        new_ds.add_column(y_name, ColumnVector::F64 { data: final_y })?;
        new_ds.add_column(&format!("{}_q1", TEMP_SUFFIX), ColumnVector::F64 { data: f_q1 })?;
        new_ds.add_column(&format!("{}_median", TEMP_SUFFIX), ColumnVector::F64 { data: f_median })?;
        new_ds.add_column(&format!("{}_q3", TEMP_SUFFIX), ColumnVector::F64 { data: f_q3 })?;
        new_ds.add_column(&format!("{}_min", TEMP_SUFFIX), ColumnVector::F64 { data: f_min })?;
        new_ds.add_column(&format!("{}_max", TEMP_SUFFIX), ColumnVector::F64 { data: f_max })?;
        new_ds.add_column(&format!("{}_sub_idx", TEMP_SUFFIX), ColumnVector::F64 { data: f_sub_idx })?;
        new_ds.add_column(&format!("{}_groups_count", TEMP_SUFFIX), ColumnVector::F64 { data: vec![groups_count; result_len] })?;
        new_ds.add_column(&format!("{}_outliers", TEMP_SUFFIX), ColumnVector::String { data: f_outliers, validity: None })?;

        if let Some(ref f) = color_field_name {
            new_ds.add_column(f, ColumnVector::String { data: final_c, validity: None })?;
        }
