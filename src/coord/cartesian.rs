...
impl Cartesian {
    ...
    pub(crate) fn new(
        ...
        Self { x_axis, y_axis, x_axis_padding_min, x_axis_padding_max, y_axis_padding_min, y_axis_padding_max}
    }
    
    // Creates a function that maps x-axis data coordinates to horizontal pixel coordinates
    pub(crate) fn x_data_to_horizontal_pixels(&self, draw_start: f64, plot_width: f64) -> impl Fn(f64) -> f64 {
    ...
    pub(crate) fn y_data_to_vertical_pixels(&self, draw_start: f64, plot_height: f64) -> impl Fn(f64) -> f64 {
        ...
        move |y| {
            ...
            if matches!(self.y_axis.scale, Scale::Discrete) {
                ...
                // Note: Pixel y-coordinates increase downward, so we invert
                draw_start + plot_height - padding_space_min - (y - min) / (max - min) * padded_height
            }
        }
    }

    // Creates a function that maps x-axis data coordinates to vertical pixel coordinates
    pub(crate) fn x_data_to_vertical_pixels(&self, draw_start: f64, plot_height: f64) -> impl Fn(f64) -> f64 {
        let ticks = self.x_axis.automatic_ticks.clone();
        move |x| {
            // For discrete scales
            if matches!(self.x_axis.scale, Scale::Discrete) {
                ...
                // Note: For vertical display, higher values should appear higher (SVG y increases downward)
                draw_start + plot_height - padding_space_min - (x - min) / (max - min) * padded_height
            }
        }
    }

    // Creates a function that maps y-axis data coordinates to horizontal pixel coordinates
    pub(crate) fn y_data_to_horizontal_pixels(&self, draw_start: f64, plot_width: f64) -> impl Fn(f64) -> f64 {
        let ticks = self.y_axis.automatic_ticks.clone();
        move |y| {
            // For discrete scales
            if matches!(self.y_axis.scale, Scale::Discrete) {
