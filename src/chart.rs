...
impl<T: Mark> Chart<T> {
    ...
    /// Calculates the raw data boundaries for standard marks where one channel 
    /// maps directly to a single data field.
    ///
    /// # Logic:
    /// 1. **Discovery**: Identifies the column name from the encoding.
    /// 2. **Type Inference**: Interprets the Polars DataType into a visual SemanticType.
    /// 3. **Range Calculation**: Finds min/max for Continuous/Temporal or unique labels for Discrete.
    /// 4. **Aesthetic Adjustments**: Forces the inclusion of zero for Bar/Area marks to 
    ///    ensure visual ratio integrity.
    fn get_standard_bounds(&self, channel: Channel) -> Result<ScaleDomain, ChartonError> {
        ...
        match semantic_type {
            SemanticType::Continuous => {
                let mut min_val = series.min::<f64>()?.unwrap_or(0.0);
                let mut max_val = series.max::<f64>()?.unwrap_or(1.0);

                // Bar-like marks should start at zero to avoid misleading viewers
            ...
            SemanticType::Temporal => {
                let min_ns = series.min::<i64>()?.unwrap_or(0);
                let max_ns = series.max::<i64>()?.unwrap_or(0);
    ...
    /// Calculates boundaries for interval-based marks (e.g., ErrorBars) by scanning 
    /// multiple related Y-axis columns to determine the true vertical extent.
    ///
    /// This method is crucial for "Implicit Encodings" where the user provides one 
    /// field (e.g., 'yield'), but the chart displays a range (e.g., yield_min to yield_max).
    ///
    /// # Search Priority:
    /// 1. Primary field (e.g., 'yield')
    /// 2. Explicit secondary field (e.g., user-defined 'y2')
    /// 3. Implicit statistical fields (e.g., '__charton_temp_yield_min/max')
    fn get_extent_bounds(&self, channel: Channel) -> Result<ScaleDomain, ChartonError> {
        let field_name = self.encoding.get_field_by_channel(channel).ok_or_else(|| {
            ChartonError::Data(format!("No field mapped to channel {:?}", channel))
        })?;

        // Interval expansion is only applicable to the Y-axis in this architecture.
        // Horizontal variations are handled via coordinate flipping later in the pipeline.
        if channel != Channel::Y {
            return self.get_standard_bounds(channel);
        }
        ...
        // Construct a list of all columns that might influence the Y-axis range.
        let mut columns_to_scan = vec![field_name.to_string()];

        // Check for explicit Y2 encoding (ggplot2 style)
        if let Some(y2_enc) = &self.encoding.y2 {
            columns_to_scan.push(y2_enc.field.clone());
        }

        // Check for implicit temp columns generated by StatTransforms (Altair style)
        columns_to_scan.push(format!("__charton_temp_{}_min", field_name));
        columns_to_scan.push(format!("__charton_temp_{}_max", field_name));

        // Iterate through all candidate columns and union their min/max values
        for col_name in &columns_to_scan {
            // .column() expects &str, so we pass a reference to our String
        ...
        // Fallback: If no columns provided valid continuous data, 
        // use the standard single-column discovery logic.
        if !found_data {
            return self.get_standard_bounds(channel);
        }

        // Final check for user-defined 'zero' preference
    ...
    Chart<T>: MarkRenderer,
{
    ...
    fn get_data_bounds(&self, channel: Channel) -> Result<ScaleDomain, ChartonError> {
        let mark_type = self.mark.as_ref().map(|m| m.mark_type()).unwrap_or("");

        // Route the request based on the Mark's nature.
        // Marks that represent intervals (errorbar/rule) need extent logic.
        match mark_type {
            "errorbar" | "rule" => self.get_extent_bounds(channel),
            _ => self.get_standard_bounds(channel),
        }
    } 
