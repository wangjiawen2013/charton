...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // The layered chart keeps the palette of its first layer, and the colour
    // domain lists that layer's categories first. So the palette is built as
    // "one colour per focus area, then grey for every context area".
